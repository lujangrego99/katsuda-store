...
export function WhatsAppButton() {
    ...
    async function fetchSettings() {
      try {
        ...
        if (res.ok) {
          ...
          setWhatsappNumber(data.data?.whatsapp || null);
        }
      } catch (error) {
  ...
  const getMessage = (): string => {
    // Product page with product info from context
    if (productInfo) {
      return `Hola! Me interesa ${productInfo.name} SKU ${productInfo.sku}`;
    }

    // Cart page
    if (pathname === '/carrito') {
      return 'Hola! Quiero consultar por mi carrito';
    }
    ...
    // Default message for home and other pages
